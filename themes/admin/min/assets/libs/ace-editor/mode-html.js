...
(function () {
    this.getCompletions = function (state, session, pos, prefix) {
        ...
        var line = session.getLine(pos.row).substr(0, pos.column);
        if (/\{\{[a-z]*$/i.test(line))
            return this.getCustomCompletions(state, session, pos, prefix);                
    ...
    this.getCustomCompletions = function (state, session, pos, prefix) {
        var values = [  'data|domain}}','data|domain_name}}','data|brand_name}}','data|report_content_email}}','data|support_request_email}}','data|vdir}}',
                        'data|member_email}}','data|member_membership}}','data|member_username}}',
                        'video-player|[search term]]}}','video-thumbs|[search term]}}',"video-results|[search term]}}","video-wall|[random]/[search term]}}"                        
                     ];
        return values.map(function (value) {
            return {
                value: value,
                //snippet: value,
                meta: "nerdyCMS bb code",
                score: 1000000
            };
        });
    };
