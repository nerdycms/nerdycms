...
$ehook = substr($this->pdata['o_hook']??$this->hook,1);

if($termt = app::request("_search",false)) {
    $terma = explode('|',$termt);
...
if($ehook!="videos") {
    $list = $ent->fetch($ehook);
} else $list = $ent->fetch($sort = app::request("_sort")??"popular");
...
{{doc}}
    <head>
        ...
        <style>
            .ovl-res {
                transform: scale(.5) translate(12rem,16rem) !important;
            }
        </style>
    </head>
