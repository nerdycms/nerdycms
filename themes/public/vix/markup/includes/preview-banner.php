...
$vurl = $a?VDIR."video?_video=$a[seo_url]":VDIR;
...
<div class="preview <?=$mode?>">
    ...
    <div id="ovt" class="overlay-text <?=$istrail?"d-none":""?>">
        <?php if($mode=="home") { 
        if(@$this->pdata['cst']['hide_banner_titles']!="on") echo "<h1 class='high-color ucase'>".@$a['title']."</h1>";
        if(!empty(@$this->pdata['cst']['video_banners_tagline'])) echo "<h2 class='ucase'>{$this->pdata['cst']['video_banners_tagline']}</h1>";
        } else if($mode=="model") { ?>
    ...
    <?php if($mode!="signup" && $mode!="signin" && $mode!="bcs") { ?>
    ...
    <?php } ?>    
    <video style="cursor: pointer" onclick="window.location.href = '<?=$vurl?>'" id="pvid" autoplay="" <?=$istrail?"":"loop=''"?> muted="" playsinline="" class="bkg" poster="<?=@$ent->meta($a,"poster")?>" src="<?=@$ent2->meta($a,$istrail?"video":"preview")?>"></video>        
</div>
